...
public class DrawShapeApp extends Application {
	...
	private void setActions() {
		...
		DrawCommand drawCommand = new DrawCommand(centerPane);
		...
		centerPane.setOnMouseClicked(e -> {
			...
			if(drawButton.isSelected()) {
				drawCommand.setCurrentShape(currentShape);
				command.execute(drawCommand.clone());
			}
		});
		
		fillColorItem.setOnAction(e->{
			changeColorCommand.setShape(selectedShape);
			command.execute(changeColorCommand.clone());
		});
		
		deleteItem.setOnAction(e->{
			deleteCommand.setShape(selectedShape);
			command.execute(deleteCommand.clone());
		});
		
		undoButton.setOnAction(e->command.undo());
		redoButton.setOnAction(e->command.redo());
